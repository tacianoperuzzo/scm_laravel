...
export const useRelatorioServicoStore = defineStore('relatorioServico', {
  ...
  actions: {
    ...
    newPreModel(userAuth) {
      this.preModel = new RelatorioSv()
      this.preModel.data_pos.setDate(this.preModel.data_pre.getDate() + 1)
      this.preModel.remetente = `${userAuth?.pessoa?.posto_graduacao?.abreviatura} ${userAuth?.pessoa?.nome_guerra}`
      this.preModel.assinatura = `${userAuth?.pessoa?.posto_graduacao?.abreviatura} ${userAuth?.pessoa?.corporacao} ${userAuth?.pessoa?.nome}\nCmt da Guarda da Casa d\'Agronômica`
    },
  },
})
