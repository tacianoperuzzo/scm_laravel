...
const novoRelatorio = () => {
    relatorioServicoStore.newPreModel(auth.user)
};
...
watch(() => props.model, async (model) => {
    if (!relatorioServicoStore.model && model) {
        relatorioServicoStore.setModel(model);
    }
})
    ...
    <DataTable v-model:filters="filters" :value="listagem" size="normal" stripedRows class="p-datatable-sm"
        ...
        <Column header="Data">
            <template #body="slotProps">
                {{ new Date(slotProps.data.data_pre).toLocaleDateString() }} para {{ new
                    Date(slotProps.data.data_pos).toLocaleDateString() }}
            </template>
        </Column>
